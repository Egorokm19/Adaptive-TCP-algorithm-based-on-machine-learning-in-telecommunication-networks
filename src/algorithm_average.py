...
class CUBIC_Algorithm(SenderAlgorithm):
    ...
    def __init__(self, c: int) -> None:
        ...
        super().__init__() # наследование параметров от класса родителя 

    def compute_w_est(self) -> int:
        """Вычисление параметров."""
        average_rtt = sum(self.rtts)/len(self.rtts) # средний RTT

        return (self.w_max * BETA_CUBIC + (
              ((3*(1 - BETA_CUBIC)) * (
...
class TAHOE_Algorithm(SenderAlgorithm):
    """Реализация алгоритма TCP Tahoe."""
...
class RENO_Algorithm(SenderAlgorithm):
    """Реализация алгоритма TCP Tahoe."""
